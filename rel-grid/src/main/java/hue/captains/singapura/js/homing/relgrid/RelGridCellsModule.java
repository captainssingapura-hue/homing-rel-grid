 ...
 * A registry from identity to the host element the grid minted and the cell
 * the domain handed over — nothing more. It asks the relation's cell manager
 * <b>once per identity</b>, mounts the cell into the host once, and from then
 * on only places and detaches. It never passes a value in, never reads one
 * out, and never disposes: a cell outlives the grid that placed it.
 */
