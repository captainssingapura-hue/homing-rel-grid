...
final class HanArticle {
    ...
    static List<String> bodyJs(boolean editor) {
        return List.of(
                ...
                "",
                "    // TWO INDEPENDENT BRANCHES: the grid's own, for everything it mints, and the",
                "    // domain's, for the squares the relation's cells mint. Dissolving both is this widget's.",
                ...
                "    var relation = createHanRelation(store, { cols: COLS, capacity: 200, branch: cellsB });",
                "",
                ...
                "        if (cols !== shownCols) { shownCols = cols; grid.viewMaps().setColumnView(relation.presentedColumns()); sizeHost(); arranged = true; }",
                "        // A run moving is an arrangement too: spans are read when cells are",
                "        // placed, so the grid is asked to place them again.",
