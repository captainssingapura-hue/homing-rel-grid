...
final class ReplicaTable {
    ...
    static List<String> bodyJs(String role) {
        return List.of(
                ...
                "",
                "    // TWO INDEPENDENT BRANCHES. The grid's OWN: everything it mints is on it, and",
                "    // it never sees the other. The DOMAIN's: the relation's cells mint their elements",
                "    // on sub-branches of 'cells', and the panels the domain draws for the grid's",
                "    // questions on sub-branches of 'panels'. Dissolving all three is this widget's.",
                "    var gridB = branch.createBranch('grid');",
                "    gridB.activate(owner);",
                "    var cellsB = branch.createBranch('cells');           // the relation's own: it activates it",
                "    var panelsB = branch.createBranch('panels');",
                ...
                "    var relation = createDishRelation(store, { role: ROLE, branch: cellsB });",
                ...
                "    // mask.panel(element) places the domain's element in the grid's box \u2014 a",
                "    // golden rectangle over the table \u2014 as a cell's element is placed in a",
                "    // slot. The choice is made there, and the answer is what the grid writes.",
                "    // It learns nothing of how it was made.",
                "    //",
                ...
                "            return dishCopyPanel(relation, question, mask, {",
                "                branch: panelsB,",
                ...
                "            return dishViewPanel(views, question, mask, { branch: panelsB });",
                "        }",
