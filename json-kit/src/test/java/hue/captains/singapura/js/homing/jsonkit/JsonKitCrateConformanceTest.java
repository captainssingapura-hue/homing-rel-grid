...
class JsonKitCrateConformanceTest {

    @Test
    void everyServedModuleIsCrated() {
        assertEquals(List.of(), OrphanCheck.check(JsonKitCrate.INSTANCE),
                "every served JS module in json-kit must be declared in its crate");
    }

    @Test
    void importsRespectCrateBoundaries() {
        assertEquals(List.of(), CrateDependencyRule.check(JsonKitCrate.INSTANCE),
    ...
    void ruleSweepAgainstAnEmptyLedger() throws IOException {
        RelGridConformanceSweep.assertLedger(JsonKitCrate.INSTANCE, Path.of("src/test/resources/rfc0044-ledger.txt"));
    }
}
