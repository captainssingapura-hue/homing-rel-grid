 ...
 * tree's caret and whatever the facade places after it. The capture surface
 * for a press, a double-click and a press on the caret, reported by position.
 */
public record RelTreeRowsModule() implements DomModule<RelTreeRowsModule> {
    ...
    @Override public ImportsFor<RelTreeRowsModule> imports() {
        return ImportsFor.<RelTreeRowsModule>builder()
                .add(new ModuleImports<>(List.of(new RelTreeStyles.hrt_row(), new RelTreeStyles.hrt_caret(), new RelTreeStyles.hrt_caret_leaf()),
                        RelTreeStyles.INSTANCE))
                .build();
    }
